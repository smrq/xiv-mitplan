...
for (const job of jobsJson) {
	const cds = job.cooldowns.map((cd) => ({
		...
		recast: 1000 * cd.recast,
		duration: 1000 * cd.duration,
	}));
...
for (const fight of fightsJson) {
	const duration =
		parseEventTime(fight.timeline[fight.timeline.length - 1].time).start +
		10000;
	...
	fights.set(fight.id, {
		...
		timeline: fight.timeline.map((marker, i) => {
			...
			const name = attack.alias || marker.name;
			const title = `${(start / 60000) | 0}:${String(
				((start % 60000) / 1000) | 0
			).padStart(2, '0')} ${marker.name}`;
			const id = `${type}-${i}`;
...
function parseEventTime(str) {
	...
	return {
		start: _parseEventTime(start),
		end: end ? _parseEventTime(end) : undefined
	};
